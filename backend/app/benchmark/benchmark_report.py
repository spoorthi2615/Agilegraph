...
class BenchmarkReport:
    ...
    def _generate_markdown(self, results: List[BenchmarkResult], path: str) -> None:
        ...
        for res in results:
            md.append(
                f"| {res.baseline_name} | {res.execution_time_ms:.2f} | {res.prediction_count} | {res.model_version} |"
            )

        md.append("")
        md.append(
            "> Raw predictions are exported to the companion CSV and JSON files for formal statistical computation in Sprint 62."
        )

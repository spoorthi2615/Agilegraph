...
from app.benchmark.execution.execution_statistics import MetricStats, BaselineStatistics, ExperimentStatistics
...
class BenchmarkExecutor:
    ...
    def execute_experiment(self, datasets: List[Tuple[TrainingDataset, CryptoGraph]]) -> ExperimentStatistics:
        """
        ...
        for dataset, graph in datasets:
            ...
            if not valid_indices:
                logger.warning(f"Dataset {dataset.project_id} has no ground truth labels. Skipping evaluation.")
                continue
            ...
            ground_truth_map = {
                sorted_nodes[i].node_id: dataset.node_labels[i] 
            ...
            for b_result in benchmark_results:
                ...
                weighted_prec = sum(p * s for p, s in zip(precisions, supports)) / total if total > 0 else 0.0
                weighted_rec = sum(r * s for r, s in zip(recalls, supports)) / total if total > 0 else 0.0
    ...
    def _export_reports(self, results: List[ExecutionResult], stats: ExperimentStatistics) -> None:
        ...
        with open(csv_path, "w", newline='') as f:
            writer = csv.writer(f)
            writer.writerow([
                "Project", "Baseline", "Accuracy", "Macro F1", "Weighted F1", 
                "Support", "Execution Time (ms)"
            ])
            for res in results:
                writer.writerow([
                    res.project_id, res.baseline_name, 
                    f"{res.accuracy:.4f}", f"{res.macro_f1:.4f}", f"{res.weighted_f1:.4f}",
                    res.support, f"{res.execution_time_ms:.2f}"
                ])
                
        # Save aggregate statistics to JSON
        json_path = os.path.join(self.config.output_directory, f"experiment_statistics_{timestamp}.json")

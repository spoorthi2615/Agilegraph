...
class CBOMComparison(BaseModel):
    ...
    unexpected_assets: List[CBOMAsset] = Field(
        default_factory=list
    )  # In CBOM but not discovered in AgileGraph
    mismatched_assets: List[Dict[str, Any]] = Field(
        default_factory=list
    )  # Found in both, but fields differ

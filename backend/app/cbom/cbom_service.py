...
class CBOMService:
    ...
    def process_and_compare(
        self, raw_json_str: str, discovered_assets: List[CBOMAsset]
    ) -> Optional[CBOMComparison]:
        """
        ...
        logger.info(
            f"Successfully parsed CBOM '{inventory.document_id}' with {len(inventory.assets)} assets."
        )

        comparison = self.comparator.compare(inventory, discovered_assets)

        logger.info(
            f"Comparison complete: {comparison.summary['matched_assets']} matched, "

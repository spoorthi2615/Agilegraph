...
class KappaReport:
    ...
    def generate(results: List[KappaResult], output_dir: str):
        ...
        with open(csv_path, "w", newline='', encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([
                "Rater A", "Rater B", "Observed Agreement", 
                "Expected Agreement", "Cohen's Kappa", "Interpretation"
            ])
            for res in results:
                writer.writerow([
                    res.rater_a, res.rater_b,
        ...
        with open(md_path, "w", encoding="utf-8") as f:
            ...
            f.write("| Rater A | Rater B | Obs. Agree (Po) | Exp. Agree (Pe) | Kappa (κ) | Interpretation |\n")
            f.write("| :--- | :--- | :--- | :--- | :--- | :--- |\n")
            for res in results:
                f.write(f"| {res.rater_a} | {res.rater_b} | {res.observed_agreement:.4f} | {res.expected_agreement:.4f} | {res.kappa_score:.4f} | **{res.interpretation}** |\n")

...
class SignificanceReport:
    ...
    def generate(results: List[SignificanceResult], output_dir: str):
        ...
        with open(csv_path, "w", newline='') as f:
            writer = csv.writer(f)
            writer.writerow([
                "Model A (Ref)", "Model B", "Metric", 
                "Observed Diff", "p-value", "Effect Size", "Alpha", "Decision"
            ])
            for res in results:
                writer.writerow([
                    res.model_a, res.model_b, res.metric_name,
                    f"{res.observed_difference:.6f}", f"{res.p_value:.6f}", 
                    f"{res.effect_size:.4f}", f"{res.alpha:.2f}", res.decision
                ])
        ...
        with open(md_path, "w") as f:
            ...
            f.write("| Reference Model | Baseline | Metric | Obs. Diff | p-value | Effect Size | Alpha | Decision |\n")
            f.write("| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |\n")
            for res in results:
                f.write(f"| {res.model_a} | {res.model_b} | {res.metric_name} | {res.observed_difference:.6f} | {res.p_value:.6f} | {res.effect_size:.4f} | {res.alpha:.2f} | **{res.decision}** |\n")

...
class ConfidenceIntervalReport:
    ...
    def generate(results: List[ConfidenceIntervalResult], output_dir: str):
        ...
        with open(csv_path, "w", newline='') as f:
            writer = csv.writer(f)
            writer.writerow([
                "Experiment", "Metric", "Mean", "Median", "Std Dev", 
                "Lower Bound", "Upper Bound", "Confidence Level", "Iterations"
            ])
            for res in results:
        ...
        with open(md_path, "w") as f:
            ...
            for res in results:
                ...
                f.write(f"| Metric | Mean | {res.metrics.get(list(res.metrics.keys())[0]).confidence_level*100:.1f}% Confidence Interval | Std Dev |\n") if res.metrics else f.write("| Metric | Mean | Confidence Interval | Std Dev |\n")
                f.write("| :--- | :--- | :--- | :--- |\n")
                for metric_name, ci in res.metrics.items():
                    f.write(f"| {metric_name} | {ci.mean:.4f} | {ci.formatted_interval} | {ci.std_dev:.4f} |\n")
                f.write("\n")

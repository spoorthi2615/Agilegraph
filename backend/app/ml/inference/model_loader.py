...
class ModelLoader:
    ...
    def load(config: InferenceConfig, in_dim: int) -> GATv2Model:
        ...
        try:
            ...
            if not arch:
                logging.warning("Checkpoint missing architecture metadata. Falling back to InferenceConfig defaults, which may cause dimension mismatch.")
            ...
            logging.info(f"Successfully loaded GATv2 model from {config.checkpoint_path} onto {config.device}")
            return model
        except Exception as e:
            logging.error(f"Failed to restore model weights from {config.checkpoint_path}: {str(e)}")

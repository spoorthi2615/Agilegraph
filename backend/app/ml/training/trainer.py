...
class GATv2Trainer:
    ...
    def _initialize_model_dims(self, data: Data):
        ...
        if model_type == "GCN":
            ...
            self.model = GCNModel(
                in_dim, self.config.hidden_dim, self.config.out_dim, self.config.dropout
            ).to(self.device)
        else:
            ...
            with torch.no_grad():
                ...
                if hasattr(val_data, "val_mask") and val_data.val_mask is not None:
                    val_loss = self.criterion(
                        val_out[val_data.val_mask], val_data.y[val_data.val_mask]
                    )
                    acc, prec, rec, f1, report_string = compute_metrics(
                        val_out[val_data.val_mask], val_data.y[val_data.val_mask]
                    )
                else:

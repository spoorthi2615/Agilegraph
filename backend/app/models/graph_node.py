...
class GraphNode(BaseModel):
    ...
    node_type: str = Field(
        ..., description="The semantic classification of the node (e.g., 'Asset', 'File')"
    )
    label: str = Field(..., description="A human-readable label or name for the node")
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Flexible key-value pairs for arbitrary node properties"
    )

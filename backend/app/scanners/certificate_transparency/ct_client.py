...
class CTClient:
    ...
    def query_domain(
        self, domain: str, include_wildcards: bool = True
        ...
        for attempt in range(1, self.config.max_retries + 1):
            try:
                logger.info(
                    f"Querying Certificate Transparency (crt.sh) for {query_domain} (Attempt {attempt}/{self.config.max_retries})"
                )

                req = urllib.request.Request(
                    url, headers={"User-Agent": "AgileGraph-CT-Scanner/1.0"}
                )
        ...
        logger.error(
            f"Failed to retrieve CT logs for {domain} after {self.config.max_retries} attempts."
        )
        return None

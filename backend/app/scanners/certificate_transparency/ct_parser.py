...
class CTParser:
    ...
    def analyze_risk(self, finding: CTFinding, active_cert_count: int) -> CTFinding:
            ...
            if now > not_after:
                ...
                if active_cert_count > 3:
                    new_finding.findings.append(
                        f"High number of active certificates detected ({active_cert_count}). Potential rogue issuance or misconfiguration."
                    )
                    score += 6.0
        ...
        if (
            "Let's Encrypt" not in new_finding.issuer
            and "DigiCert" not in new_finding.issuer
            and "GlobalSign" not in new_finding.issuer
        ):

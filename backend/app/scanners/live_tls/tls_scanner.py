...
class TLSScanner:
    ...
    def analyze_risk(self, finding: TLSFinding) -> TLSFinding:
        ...
        if new_finding.tls_version in ["TLSv1", "TLSv1.1", "SSLv3", "SSLv2"]:
            new_finding.findings.append(f"Unsupported/Deprecated Protocol: {new_finding.tls_version}")
            score += 10.0
            
        if "RC4" in new_finding.cipher_suite or "DES" in new_finding.cipher_suite or "MD5" in new_finding.cipher_suite:

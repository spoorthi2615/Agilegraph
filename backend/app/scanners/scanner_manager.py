...
class ScannerManager:
    ...
    def execute_all(
        self, project_path: Path, target_languages: Optional[List[str]] = None
    ) -> List[ScannerResult]:
        """
            ...
            try:
                ...
                with ThreadPoolExecutor(max_workers=1) as executor:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Scanner '{name}' timed out after {SCANNER_TIMEOUT_SECONDS}s — skipping."
                        )
                        future.cancel()
                        results.append(
                            ScannerResult(
                                ...
                                findings=[],
                                errors=[
                                    f"Scanner timed out after {SCANNER_TIMEOUT_SECONDS} seconds."
                                ],

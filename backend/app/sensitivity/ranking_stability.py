...
class RankingStabilityEngine:
    ...
    """
    def calculate_stability(self, perturbation_id: str, base_ranking: List[str], perturbed_ranking: List[str]) -> RankingStabilityResult:
        n = len(base_ranking)
        
        if n == 0:
            return RankingStabilityResult(
                perturbation_id=perturbation_id,
                spearman_rho=1.0, top_10_overlap=1.0, top_20_overlap=1.0,
                mean_rank_difference=0.0, max_rank_difference=0
            )

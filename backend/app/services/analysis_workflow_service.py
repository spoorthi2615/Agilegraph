    ...
    def execute_pipeline(
        ...
        try:
            ...
            try:
                ...
                _model_cached = (
                    _Path(_hf_cache) / "hub" / "models--microsoft--codebert-base"
                ).exists()

                if not _model_cached:
                    logging.info(
                        f"[{project_id}] CodeBERT not cached locally — skipping ML inference, using heuristic scores."
                    )
                ...
                inf_result = GATv2InferenceService.run_inference(
                    loaded_model, inf_dataset, inf_config
                )
                ...
                logging.warning(
                    f"[{project_id}] ML Inference skipped (using heuristic scores): {ml_err}"
                )
            ...
            try:
                ...
                logging.warning(
                    f"[{project_id}] Neo4j export failed (scan still marked complete): {neo4j_err}"
                )

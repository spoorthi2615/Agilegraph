...
class DatasetValidationService:
    ...
    def validate(cls, dataset: TrainingDataset) -> DatasetValidation:
        ...
        if not dataset.node_features:
            ...
            for idx, f_vec in enumerate(dataset.node_features):
                if len(f_vec) != feature_dim:
                    messages.append(
                        f"Inconsistent feature dimension at node index {idx}. Expected {feature_dim}, got {len(f_vec)}."
                    )
                    is_valid = False
        ...
        if len(dataset.node_labels) != dataset.total_nodes:
            messages.append(
                f"Label count mismatch. Expected {dataset.total_nodes}, got {len(dataset.node_labels)}."
            )
            is_valid = False
        ...
        for edge_idx, (src, tgt) in enumerate(dataset.edge_index):
            if src < 0 or src >= dataset.total_nodes or tgt < 0 or tgt >= dataset.total_nodes:
                messages.append(
                    f"Malformed edge at index {edge_idx}: ({src}, {tgt}) references out-of-bounds nodes."
                )
                is_valid = False
            else:
        ...
        if isolated_nodes > 0:
            ...
            messages.append(
                f"Detected {isolated_nodes} isolated nodes with no topological connections."
            )

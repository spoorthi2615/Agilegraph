...
class FleissKappaService:
    ...
    def calculate_kappa(cls, validations: List[ExpertValidation]) -> MultiRaterReliability:
        ...
        if not validations or len(validations) < 3:
            raise ValueError(
                f"Fleiss' Kappa mathematically requires at least 3 experts. Found {len(validations)}."
            )
        ...
        if len(set(expert_ids)) != len(expert_ids):
            raise ValueError(
                "Duplicate expert IDs found. Each validation must originate from a distinct expert."
            )
        ...
        return MultiRaterReliability(
            ...
            metadata={"validation_ids": [str(v.validation_id) for v in validations]},
        )

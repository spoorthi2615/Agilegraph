...
class GATv2InferenceService:
    ...
    @classmethod
    def run_inference(
        cls, model: Any, dataset: InferenceDataset, config: ModelConfig
    ) -> InferenceResult:
        """
        ...
        if hasattr(config, "classification_threshold"):
            ...
            and config.model_extra
            and "classification_threshold" in config.model_extra
        ):
        ...
        if not isinstance(model, nn.Module):
            raise TypeError(
                "Provided model instance is not a valid PyTorch nn.Module. Cannot run inference."
            )
        ...
        if not dataset.edge_index:
            raise ValueError(
                "Inference Dataset contains no topological edges. Message passing is impossible."
            )

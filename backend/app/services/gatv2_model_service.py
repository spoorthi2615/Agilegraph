...
class GATv2ModelService:
    ...
    def initialize_model(cls, dataset: TrainingDataset, config: ModelConfig) -> Any:
        ...
        if TORCH_AVAILABLE:
            logger.info(
                f"Initializing native PyTorch GATv2 architecture with {config.input_dimension} input dims."
            )
            return GATv2NodeClassifier(config)
        else:
            # Fallback for core backend environments where PyTorch is intentionally excluded
            logger.warning(
                "PyTorch Geometric is not installed. Returning symbolic model initialization."
            )
            return {

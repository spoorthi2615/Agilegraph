            ...
            with zipfile.ZipFile(zip_path, "r") as zip_ref:
                ...
                if len(infolist) > MAX_FILES_IN_ARCHIVE:
                    raise EntityTooLargeException(
                        f"Archive contains too many files (max {MAX_FILES_IN_ARCHIVE})."
                    )
                ...
                for zinfo in infolist:
                    ...
                    if zinfo.filename.startswith("/") or zinfo.filename.startswith("\\"):
                        raise ValidationException(
                            "Archive contains absolute paths which are not allowed."
                        )

                    # 1. Prevent ZIP Slip (Directory Traversal)
                    resolved_target_path = os.path.abspath(
                        os.path.join(resolved_source_dir, zinfo.filename)
                    )
                    ...
                    if (
                        not resolved_target_path.startswith(resolved_source_dir + os.sep)
                        and resolved_target_path != resolved_source_dir
                    ):
                        raise ValidationException(
                            "Archive attempts directory traversal (ZIP Slip)."
                        )
                    ...
                    if mode and stat.S_ISLNK(mode):
                        raise ValidationException(
                            "Archive contains symbolic links which are not allowed."
                        )

                    # 4. Reject entries larger than 100 MB after extraction
                    if zinfo.file_size > MAX_EXTRACTED_FILE_SIZE:
                        raise EntityTooLargeException(
                            f"Archive contains an uncompressed file exceeding the {MAX_EXTRACTED_FILE_SIZE // (1024*1024)}MB limit."
                        )
                    ...
                    if cumulative_size > MAX_CUMULATIVE_EXTRACTED_SIZE:
                        raise EntityTooLargeException(
                            f"Archive exceeds the maximum cumulative extracted size of {MAX_CUMULATIVE_EXTRACTED_SIZE // (1024*1024)}MB."
                        )

...
class RiskCalculator:
    ...
    def calculate_base_risk(self, algorithm: str, data_shelf_life_years: int, migration_time_years: int, data_sensitivity_score: float = 1.0) -> float:
        """
        ...
        if algo_upper in explicit_weak:
            ...
            raise ValueError(f"Unclassified algorithm detected: {algorithm}. Cannot silently fall through.")
    ...
    def evaluate_certificate_risk(self, expiry_date_str: str, algorithm: str) -> float:
        ...
        base_risk = self.calculate_base_risk(algorithm, data_shelf_life_years=1, migration_time_years=0.5)
        return base_risk

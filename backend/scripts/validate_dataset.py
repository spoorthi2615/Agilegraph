...
def validate():
    ...
    for filename in os.listdir(DATASETS_DIR):
        ...
        for edge in edges:
            if edge.get("source_node") not in node_ids:
                graph_violations.append(f"{repo_id}: Dangling Edge source {edge.get('source_node')}")
            if edge.get("target_node") not in node_ids:
                graph_violations.append(f"{repo_id}: Dangling Edge target {edge.get('target_node')}")
        ...
        if len(node_features) != len(nodes):
            dataset_violations.append(f"{repo_id}: Feature vector length ({len(node_features)}) != nodes length ({len(nodes)})")
            
        if len(labels) != len(nodes):
            dataset_violations.append(f"{repo_id}: Labels length ({len(labels)}) != nodes length ({len(nodes)})")
        ...
        if node_features:
            ...
            for i, vec in enumerate(node_features):
                if len(vec) != dim:
                    dataset_violations.append(f"{repo_id}: Inconsistent feature dimensions at node index {i}")
                for val in vec:
            ...
            if len(nodes) > 0:
                dataset_violations.append(f"{repo_id}: Feature vectors empty but graph has {len(nodes)} nodes")
    ...
    with open(report_path, "w") as f:
        ...
        if risk_dist["0"] > total_risk * 10:
            f.write("> **WARNING: Severe Class Imbalance.** Safe nodes heavily outnumber risky nodes. ")
            f.write("It is highly recommended to use **Focal Loss** or apply Node Sampling techniques during GATv2 training.\n\n")
        else:
            f.write("> Class balance is acceptable for GNN training.\n\n")
            
        f.write("## 4. Overall Readiness\n")
        score = 10
        if total_repos - processed_repos > 0: score -= 1
        if len(graph_violations) > 0: score -= 3
        if len(dataset_violations) > 0: score -= 3
        if risk_dist["0"] > total_risk * 10: score -= 1
        
        f.write(f"### Readiness Score: {max(0, score)} / 10\n\n")
        if score >= 8:
            f.write("**Status: READY FOR TRAINING.** The dataset is mathematically sound and ready to be loaded by PyTorch Geometric.\n")
        else:
            f.write("**Status: ACTION REQUIRED.** Please address the structural violations before proceeding to training.\n")

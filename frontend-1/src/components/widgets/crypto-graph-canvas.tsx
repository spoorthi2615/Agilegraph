...
export const CryptoGraphCanvas = React.memo(function CryptoGraphCanvas({ 
  nodes, edges, zoom, setZoom, selected, onSelect 
}: { 
  nodes: GraphNode[]; edges: GraphEdge[]; zoom: number; setZoom: (z: number | ((z: number) => number)) => void; selected: string | null; onSelect: (id: string) => void 
}) {
    ...
    >
      <svg viewBox="0 0 1000 640" className="h-full w-full">
        ...
        {edges.map((e, i) => {
          ...
          if (!s || !t) return null;
          return <line key={i} x1={s.x} y1={s.y} x2={t.x} y2={t.y} stroke="var(--color-muted-foreground)" strokeWidth="1.5" opacity="0.4" />;
        })}
        {nodes.map((n) => {

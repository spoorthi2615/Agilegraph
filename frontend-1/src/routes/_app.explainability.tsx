...
export const Route = createFileRoute("/_app/explainability")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Explainable AI rationale for cryptographic risk scores and PQC recommendations.",
      },
    ],
  }),
});
...
function Explainability() {
  ...
  if (isLoading || !assets.length || !explainData)
    return (
      <div className="p-8 text-center text-muted-foreground">Loading explainability data...</div>
    );

  const {
    assetInformation: info,
    gnnExplanation: gnn,
    heuristicExplanation: heur,
    migrationRecommendation: mig,
  ...
  const factors =
    gnn?.featureImportance?.map((f: FeatureItem) => ({
      label: f.featureName,
      ...
      <main className="p-4 md:p-6 space-y-6">
        <section className="grid gap-6 lg:grid-cols-[380px_1fr]">
          ...
          <div className="rounded-xl border bg-card p-6">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold">Why was this asset ranked high?</h3>
            </div>
            <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
              {naturalLanguageSummary ||
                "AgileGraph's risk model combines intrinsic algorithmic weakness with graph-based exposure and business context to produce this score."}
            </p>
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          <div className="rounded-xl border bg-card p-6">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold">Graph Influence</h3>
            </div>
            <p className="mt-2 text-xs text-muted-foreground">
              Direct + transitive downstream dependents.
            </p>
            <div className="mt-4 grid place-items-center">
              <svg viewBox="0 0 260 180" className="w-full">
                ...
                {[
                  [60, 40, "var(--color-warning)"],
                  [200, 40, "var(--color-warning)"],
                  [40, 140, "var(--color-chart-5)"],
                  [130, 160, "var(--color-success)"],
                  [220, 140, "var(--color-warning)"],
                ].map((c, i) => (
                  <circle
                    key={i}
                    cx={c[0] as number}
            ...
            <div className="mt-2 grid grid-cols-3 gap-2 text-center text-xs">
              ...
              <div>
                ...
                <div className="text-muted-foreground">Transitive</div>
              </div>
              <div>
            ...
            <div className="mt-4 space-y-4 text-sm">
              <div className="rounded-lg border-l-4 border-primary bg-primary/5 p-4">
                <div className="text-xs font-semibold uppercase tracking-widest text-primary">
                  Model Confidence:{" "}
                  {Math.round((explainData?.confidenceMetrics?.overallConfidence || 0) * 100)}%
                </div>
                <p className="mt-2 leading-relaxed">
                  Migrate to{" "}
                  <span className="font-mono font-semibold">
                    {mig?.recommendedPqcAlgorithm || "Unknown"}
                  </span>
                  . Estimated effort: {mig?.migrationEffort || 0} days. Risk reduction:{" "}
                  {mig?.estimatedRiskReduction || 0}%.
                </p>
              </div>
              <div className="rounded-lg border p-4">
                ...
                <ol className="mt-3 space-y-2 text-sm text-muted-foreground">
                  <li>
                    ...
                    {heur?.breakdown?.riskFormulaBreakdown || "Heuristics computed."}
                  </li>
                  <li>
                    <span className="mr-2 font-mono text-primary">2.</span>{" "}
                    {heur?.breakdown?.penaltyBreakdown || "Penalties applied."}
                  </li>
                </ol>
              </div>
            </div>
        ...
        <section className="rounded-xl border bg-card p-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold">Timeline of Reasoning</h3>
          </div>
          <ol className="mt-6 grid gap-6 md:grid-cols-4">
            {[
              ...
              {
                t: "Recommendation",
                d: `PQC upgrade to ${mig?.recommendedPqcAlgorithm || asset.recommended}`,
                ok: true,
              },
            ].map((s, i) => (

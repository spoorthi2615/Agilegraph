...
export const Route = createFileRoute("/_app/mosca")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Assess PQC readiness using Mosca's inequality across your critical assets.",
      },
    ],
  }),
});
...
function Mosca() {
  ...
  const status =
    surplus < 0
      ? { label: "At risk — migrate now", color: "var(--color-critical)" }
      : surplus < 2
        ? { label: "Tight — start immediately", color: "var(--color-warning)" }
        ...
        {isLoading ? (
          ...
          <div className="flex h-[400px] flex-col items-center justify-center rounded-xl border border-dashed text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Upload and scan a repository to compute your Mosca readiness score.
            </p>
          </div>
        ) : (
          <div className="grid gap-6 lg:grid-cols-[1fr_400px]">
            <div className="space-y-4">
              <div className="rounded-xl border bg-card p-6">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  If confidentiality lifetime (X) plus migration duration (Y) exceeds quantum
                  horizon (Z), your data is at risk.
                </p>

                <div className="mt-8 space-y-8">
                  <SliderInput
                    label="Quantum Horizon (Z)"
                    hint="Estimated years until a CRQC exists"
                    value={z}
                    onChange={setZ}
                    max={30}
                    suffix="years"
                  ...
                  <div className="rounded-md border p-4 bg-muted/20">
                    ...
                    <div className="mt-2 text-xs text-muted-foreground">
                      Derived from actual repository analysis in Neo4j.
                    </div>
                ...
                <div className="mt-8 rounded-lg border bg-muted/30 p-5">
                  ...
                  <div className="mt-3 text-sm">
                    ...
                    <span className="font-semibold" style={{ color: status.color }}>
                      {surplus > 0 ? `+${surplus}` : surplus} years
                    </span>{" "}
            ...
            <aside className="space-y-4">
              <div className="rounded-xl border bg-card p-6 text-center">
                ...
                <div className="mt-2 text-sm font-medium" style={{ color: status.color }}>
                  {status.label}
                </div>
              </div>
              ...
              <div className="rounded-xl border bg-card p-6">
                ...
                <div className="mt-4 space-y-3">
                  <Bar
                    label="Data must stay secret"
                    years={x}
                    color="var(--color-primary)"
                    max={Math.max(x + y, z, 15)}
                  />
                  <Bar
                ...
                <p className="mt-4 flex items-start gap-2 text-xs text-muted-foreground">
                  ...
                  Based on Michele Mosca's inequality (2015). Adjust inputs to model different
                  scenarios.
                </p>
              </div>
            </aside>
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between">
        ...
        <div className="text-2xl font-semibold tabular-nums" style={{ color: tint }}>
  ...
  return (
    <div>
      <div className="mb-1 flex justify-between text-xs">
        ...
        <span className="font-medium">{years}y</span>
      </div>
      <div className="h-2 rounded-full bg-muted overflow-hidden">
        <div
          className="h-full rounded-full transition-all duration-500"
          style={{ width: `${(years / max) * 100}%`, background: color }}
        />
      </div>
    </div>
  );

...
export const Route = createFileRoute("/_app/reports")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Generate executive, technical, migration, and risk reports.",
      },
    ],
  }),
});
...
const generators = [
  {
    ...
    body: "Board-ready summary with KPIs and migration progress.",
    icon: FileBarChart,
  ...
  },
  {
    ...
    body: "Full inventory with algorithms, dependencies, and remediation.",
    icon: FileText,
    tint: "from-[oklch(0.7_0.18_300)]/15 to-transparent",
  },
  {
    key: "mig",
    title: "Migration Plan",
    body: "Sequenced roadmap with owners, effort, and PQC recommendations.",
    icon: RouteIcon,
    tint: "from-success/15 to-transparent",
  },
  {
    key: "risk",
    title: "Risk Summary",
    body: "Prioritized critical exposures and Mosca readiness posture.",
    icon: ShieldAlert,
    tint: "from-critical/15 to-transparent",
  },
];
  ...
  const handleDownload = async (type: string, format: string) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/reports/latest/download?format=${format}&type=${encodeURIComponent(type)}`,
        {
          method: "GET",
          headers,
        },
      ...
      <main className="p-4 md:p-6 space-y-6">
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {generators.map((g) => (
            <div
              key={g.key}
              className={`card-hover animate-fade-up rounded-2xl border bg-gradient-to-br ${g.tint} bg-card p-5`}
            >
              ...
              <div className="mt-4 flex flex-wrap gap-1.5">
                <Button size="sm" variant="outline" onClick={() => handleDownload(g.title, "pdf")}>
                  <Download className="h-3.5 w-3.5" />
                  PDF
                </Button>
                <Button size="sm" variant="outline" onClick={() => handleDownload(g.title, "csv")}>
                ...
                <Button
                  ...
                  onClick={() => {
                    window.print();
                    toast.success(`Printing ${g.title}`);
                  }}
                >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-muted/30 text-xs text-muted-foreground">
                <tr>
                  {["Title", "Type", "Created", "Size", "Author", ""].map((h) => (

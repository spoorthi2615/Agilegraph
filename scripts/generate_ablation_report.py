...
**Finding**: The F1 score drops significantly from **{full_f1}** to **{no_heur_f1}** when the heuristic feature is removed. 
...
Removing the Graph Attention mechanism (GATv2) and replacing it with a standard GCN convolution resulted in an F1 of **{no_gat_f1}**. This indicates that dynamic attention weighting over neighboring nodes provides minimal to no benefit for this specific task compared to standard symmetric normalization.

### Relational Edges (- Heterogeneous)
Treating all edges identically (Homogeneous GCN) resulted in an F1 of **{no_het_f1}**, which actually outperformed the Full Model. This suggests that separating edge types (Calls, Inherits, Imports) into a heterogeneous graph structure added unnecessary noise or caused overfitting, rather than providing useful structural priors.

### Semantic Features (- CodeBERT)
Replacing the 768-dimensional CodeBERT embeddings with random noise vectors caused the F1 score to drop to **{no_codebert_f1}**. This confirms that the semantic meaning of the code tokens is strictly necessary for the network to make any meaningful predictions, and the graph structure alone (without node content) is insufficient.
